...
-- Expand tabs to spaces.  When set, a <Tab> in front of a line inserts spaces instead of a tab.  When unset, a <Tab> in front of a line inserts a real tab.
...
vim.o.foldlevel = 99
vim.o.ai = true -- Enable automatic indenting
vim.o.si = true -- Enable smart indenting
...
-- Enable 24-bit RGB color in the TUI (required for custom cursor colors)
vim.o.termguicolors = true

-- Show which line your cursor is on
vim.o.cursorline = true

-- Set cursor shape and use Cursor/iCursor highlight groups for color
vim.o.guicursor = "n-v-c:block-Cursor/lCursor,i-ci-ve:ver25-iCursor/lCursor,r-cr:hor20-iCursor/lCursor,o:hor50"
...
vim.keymap.set("n", "<leader>ba", ":bufdo bd<CR>", { desc = "[B]uffer [A]ll delete" })
...
require("lazy").setup({
	...
	{ -- Highlight, edit, and navigate code
		"nvim-treesitter/nvim-treesitter",
		-- `main` is the actively-maintained rewrite and the only branch compatible
		-- with Neovim 0.11+. The old `master` branch is frozen and its markdown
		-- injection query crashes the treesitter core on 0.12 (nil `node:range`).
		-- `main` drops the `nvim-treesitter.configs` opts API: parsers are installed
		-- with `.install{}` and features are enabled per-buffer in a FileType autocmd.
		branch = "main",
		lazy = false, -- nvim-treesitter does not support lazy-loading
		build = ":TSUpdate",
		-- [[ Configure Treesitter ]] See `:help nvim-treesitter`
		config = function()
			-- Install the parsers we rely on (async; no-op if already installed).
			...
			-- language has an installed parser. Replaces the old `highlight`/`indent`
			-- opts. `vim.treesitter.start` throws when no parser is available, so it
			-- is guarded with pcall.
		...
		config = function()
			require("scratch").setup({
				...
				filetype_details = { -- or, you can have more control here
					["ftron"] = {
						subdir = "ftron", -- group scratch files under specific sub folder
						type = "md",
					},
					go = {
	...
	-- { "flazz/vim-colorschemes", lazy = true }, -- Collection of colorschemes
